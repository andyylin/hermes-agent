...
async def get_profiles_sessions(
    ...
    Andy runtime overlay: cron sessions are internal automation transcripts, not
    human chats. Hide them from this user-facing aggregate list by default while
    leaving them queryable in the session database for audit/debug.
    """
    if archived not in ("exclude", "only", "include"):
    ...
    # newest cron sessions can't starve the recents page. Andy runtime default:
    # when no explicit source scope is requested, hide cron automation transcripts
    # from the user-facing aggregate profile list.
    source_filter = source or None
    exclude_list = [s for s in (exclude_sources or "").split(",") if s.strip()]
    if not source_filter and not exclude_list:
        exclude_list = ["cron"]
