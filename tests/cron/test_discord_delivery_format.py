...
def test_mixed_target_prompt_has_no_discord_only_generation_guidance():
    ...
    with patch.object(scheduler, "_resolve_delivery_targets", return_value=targets):
        prompt = scheduler._build_job_prompt(job)

    assert "DISCORD FORMAT" not in prompt
    assert "Do not use Markdown tables" not in prompt


def test_wrap_response_false_preserves_exact_discord_content():
    job = {"id": "raw-report", "name": "Raw report", "deliver": "discord"}
    content = "| Item | Status |\n| --- | --- |\n| Backup | Healthy |"
    target = {"platform": "discord", "chat_id": "discord-room"}
    gateway_cfg = MagicMock(
        platforms={Platform.DISCORD: MagicMock(enabled=True, extra={})}
    )
    send_mock = AsyncMock(return_value={"success": True})

    with (
        patch.object(scheduler, "_resolve_delivery_targets", return_value=[target]),
        patch.object(scheduler, "_resolve_origin", return_value=None),
    ...
    assert send_mock.await_args_list[0].args[3] == content
