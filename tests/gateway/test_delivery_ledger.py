...
class TestGatewayRedeliverySweep:
    ...
    async def test_missing_adapter_leaves_row_recoverable(self):
        ...
        assert n == 0
        # No adapter means no claim is taken. The dead owner stamp remains as
        # crash evidence, while a later scoped reconnect can claim and send it
        # without having consumed the attempt budget.
        assert _row("ob-1")["state"] == "pending"
        assert _row("ob-1")["attempts"] == 0
        assert _row("ob-1")["owner_pid"] == 999999999
