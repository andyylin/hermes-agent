...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # Sorted: ["kanban", "memory", "memory_tree", "project"]. `kanban` is auto-recovered by
    # _get_platform_tools (a non-configurable platform toolset in hermes-cli's
    # universe); `memory_tree` is retained runtime policy; `project` is GUI-only,
    # folded in by _load_enabled_toolsets.
    assert server._load_enabled_toolsets() == [
        "kanban", "memory", "memory_tree", "project"
    ]
