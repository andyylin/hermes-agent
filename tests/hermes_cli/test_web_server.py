...
class TestWebServerEndpoints:
    ...
    def test_profiles_sessions_hides_cron_runs(self):
        """Cron transcripts are automation internals, not desktop chat rows."""
        from hermes_state import SessionDB

        db = SessionDB()
        try:
            db.create_session(session_id="cron-noise", source="cron")
            db.append_message(session_id="cron-noise", role="user", content="[SILENT]")
            db.create_session(session_id="human-chat", source="discord")
            db.append_message(session_id="human-chat", role="user", content="show me")
        finally:
            db.close()
        ...
        assert "human-chat" in ids
        assert "cron-noise" not in ids
