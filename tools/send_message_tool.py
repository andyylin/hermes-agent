...
async def _registry_standalone_send(platform_name, pconfig, chat_id, message, thread_id=None, **metadata):
    ...
    ``metadata`` intentionally carries platform-specific one-shot options such
    as Email ``subject`` through the plugin migration boundary without
    reintroducing per-platform core send helpers.
    """
    from gateway.platform_registry import platform_registry
    ...
    return await entry.standalone_sender_fn(pconfig, chat_id, message, thread_id=thread_id, **metadata)
...
async def _send_email(extra, chat_id, message, subject=None):
    """Backward-compatible email one-shot helper.

    Email delivery moved to the bundled plugin; keep this thin wrapper for
    legacy tests/callers while the actual implementation lives at the plugin
    boundary.
    """
    ...
    return await _standalone_send(
        ...
        message,
        subject=subject,
        suppress_threading=bool(subject),
    )


# _send_sms moved to plugins/platforms/sms/adapter.py::_standalone_send. It is
# wired via standalone_sender_fn and reached through _registry_standalone_send. #41112.
