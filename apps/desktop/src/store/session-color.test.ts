...
import { $projects, $sessionProjectAssignments } from './projects'
...
describe('$sessionColorById', () => {
  ...
  it('prefers an explicit project assignment over cwd-derived ownership', () => {
    const session = makeSession('/www/app/src', { git_repo_root: '/www/app' })

    $projects.set([
      makeProject('p_app', ['/www/app'], '#4a9eff'),
      makeProject('p_target', ['/other/project'], '#ff8c42')
    ])
    $sessionProjectAssignments.set({ [session.id]: 'p_target' })
    $sessions.set([session])

    expect($sessionColorById.get()[session.id]).toBe('#ff8c42')
  })

  it('treats an explicit No Project assignment as authoritative', () => {
    const session = makeSession('/www/app/src', { git_repo_root: '/www/app' })

    $projects.set([makeProject('p_app', ['/www/app'], '#4a9eff')])
    $sessionProjectAssignments.set({ [session.id]: null })
    $sessions.set([session])

    expect($sessionColorById.get()[session.id]).toBeUndefined()
  })
})
